...
class Reminder:
    ...
    async def send_reminder(self, ctx):
        ...
        if time_length < 0:
            ...
            embed = discord.Embed(
                title=self.title, description=self.info, color=discord.Colour.purple()
            )
            time_remaining = (
                f"{int(days[0])} days, {int(hours[0])} hours,"
                f" {int(minutes[0])} minutes"
            )
            embed.add_field(
                name="Event Time/Date", value=self.sched_time.strftime("%c")
            )
            embed.add_field(name="Time remaining", value=time_remaining)
            embed.set_footer(
                text="React to this message to get notified when the event starts!"
            )
    ...
    async def trigger_notify(self):
        ...
        message = await self.bot.get_channel(self.message.channel.id).fetch_message(
            self.message.id
        )
        if message is None:
            self.complete = True
    ...
    async def trigger_notify_all(self):
        """
        Notifies all uses in channel regardless of reaction when
        reminder/event has started.
        """
        self.complete = True
    ...
    async def update_reminder(self):
        ...
        minutes = divmod(hours[1], 60)
        time_remaining = (
            f"{int(days[0])} days, {int(hours[0])} hours," f" {int(minutes[0])} minutes"
        )
...
class ReminderScheduler:
    ...
    async def remove_reminder(self, ctx, title: str):
        ...
        message = await self.bot.get_channel(old_message.channel.id).fetch_message(
            old_message.id
        )
